...
SELECT `facid`, `name`, `membercost`, `guestcost`, `initialoutlay`, `monthlymaintenance` FROM `Facilities` WHERE 20*(`monthlymaintenance`)/100>`membercost`
...
SELECT temp_table.surname as Member,
	temp_table.name as Facility,
	temp_table.cost as cost
FROM
...
WHERE temp_table.starttime >= '2012-09-14'
AND temp_table.starttime < '2012-09-15'
AND temp_table.cost > 30
